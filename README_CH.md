...
| 依赖名称 | 版本范围 | 功能描述 |
| --- | --- | --- |
| **fastapi** | `>=0.115.12,<0.116.0` | **Web 框架**：提供高性能的 API 服务接口，支持异步编程。 |
| **mineru** | `>=3.0.0,<3.2.0` | **核心引擎**：MinerU 核心库，提供 PDF 解析、布局分析和公式提取能力。 |
| **celery** | `>=5.5.3,<6.0.0` | **异步任务队列**：用于处理耗时的 PDF 解析和 NLP任务。 |
| **vllm** | `>=0.11.0,<0.12.0` | **大模型推理**：高性能 LLM/VLM 推理引擎，用于加速VLM视觉大模型的推理。 |
| **transformers** | `>=4.53.0,<5.0.0` | **NLP 模型库**：用于加载和运行各种预训练模型（如 NER、Embedding）。 |
| **minio** | `>=7.2.15,<8.0.0` | **对象存储客户端**：用于上传和下载 PDF 源文件及解析后的结果文件。 |
| **sentence-transformers** | `>=5.1.0,<6.0.0` | **文本向量化**：用于生成文本 Embedding，实现基于文本语义的切分。 |
| **sqlalchemy** | `>=2.0.41,<3.0.0` | **ORM 框架**：用于数据库操作和连接管理。 |
| **redis** | 外部服务（Docker 镜像 `redis:alpine`） | **缓存与消息中间件**：作为 Celery 的 Broker 和 Backend，同时提供运行时缓存。 |
